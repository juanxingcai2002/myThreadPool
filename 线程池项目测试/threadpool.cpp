...
void ThreadPool::threadFunc(int threadid)
{
	...
	for(;;)//线程池中的线程应该是一直执行着这个动作（从任务队列中取任务然后执行），因此应该放在死循环中，且放在死循环中，即可保证计算pool对象被析构了，但正在执行任务的线程会执行完所有的任务，只有taskQue.size() ==0 
		  //所有的线程都执行完了，才会去判断线程池的状态isPoolRunning.
	{
		...
		{// 获取锁
			...
			// 锁 + 双重 判断 ：因为不知道在线程池析构的时候，是线程池的析构函数先获得锁（会改变isPoolRunning的值）还是线程函数先获得这把锁
			while (taskQue_.size() == 0)// 说明没有任务，工作线程就应该阻塞在这。
			{
				...
				if (isPoolRunning_ == false)
				{
					...
					exitCond_.notify_all(); // 唤醒阻塞在exitCond上的线程，但不一定能解除阻塞，具体还要看第二个参数的条件是否能得到满足。
					return;//线程函数结束，线程结束
				}
			...
			//if (!isPoolRunning_) //线程池被析构了--直接退出外层的while --执行最后的线程删除函数。
			//{
			//	break; 
			//}
